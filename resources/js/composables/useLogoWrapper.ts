...
export function useLogoWrapper() {
    function svgLogoWrapper(node: any, className?: string): () => VNode {
        return () => h('div', {
            class: cn(
                "flex items-center gap-3 h-7 [&>svg]:h-7 [&>svg]:w-auto select-none transition-all duration-200",
                "grayscale opacity-60 dark:opacity-50 dark:brightness-0 dark:invert",
                "hover:grayscale-0 hover:opacity-100 hover:dark:opacity-100 hover:dark:brightness-100 hover:dark:invert-0",
                className
            )
        }, [
            h(node)

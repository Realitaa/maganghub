...
const inviteUrl = computed(
    () => props.group.invite_url ?? `${window.location.origin}/join/${props.group.code}`,
);
    ...
    <Dialog :open="open" @update:open="$emit('update:open', $event)">
        <DialogContent class="sm:max-w-md">
            <DialogHeader>
                ...
                <DialogDescription>
                    Bagikan kode atau link undangan kepada calon anggota kelompok.
                    Saat dibagikan di WhatsApp, preview gambar akan tampil otomatis.
                </DialogDescription>
            </DialogHeader>

            <!-- WhatsApp OG Preview Mockup -->
            <div class="overflow-hidden rounded-xl border border-border/60 shadow-sm">
                    ...
                    >
                        <div class="flex flex-col items-center gap-2 text-muted-foreground/30">
                            <svg class="h-10 w-10" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909M3.75 18h16.5M12 3v2.25m0 0V3m0 2.25H9.75m2.25 0H14.25" />
                            </svg>
                ...
                <div class="bg-muted/60 p-3">
                    <p class="truncate text-[10px] uppercase tracking-wider text-muted-foreground">
                        {{ inviteUrl }}
                    </p>
                    <p class="mt-0.5 line-clamp-2 text-sm font-semibold leading-snug">
                        Bergabung ke Kelompok Magang {{ group.leader.name }}
                    </p>
                    <p class="mt-0.5 truncate text-xs text-muted-foreground">
                        Gunakan kode <span class="font-mono font-semibold">{{ group.code }}</span> untuk bergabung.
                    </p>
                </div>
            </div>
            ...
            <div class="flex flex-col items-center gap-2 py-2">
                ...
                <p class="font-mono text-4xl font-bold tracking-[0.25em] text-primary">
                    {{ group.code }}
                </p>
            </div>
            ...
            <div class="flex flex-col gap-2">
                <Button
                    ...
                    @click="copyCode()"
                >
                    <component :is="codeCopied ? Check : Copy" class="h-4 w-4" />
                ...
                <Button
                    ...
                    @click="copyLink()"
                >
                    <component :is="linkCopied ? Check : Link2" class="h-4 w-4" />
